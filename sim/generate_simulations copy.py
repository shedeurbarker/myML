...
def get_next_sim_folder():
    """Get the next available simulation folder name."""
    base_dir = os.path.join(os.path.dirname(__file__), 'simulations')
    index = 1
    while True:
        folder_name = os.path.join(base_dir, f'sim_{index:04d}')
...
def main():
    # Get the next available simulation folder
    sim_folder = get_next_sim_folder()
    logging.info(f"Creating simulation folder: {sim_folder}")
    os.makedirs(sim_folder, exist_ok=True)

    # Generate parameter combinations
    combinations = generate_parameter_combinations()
    for i, combo in enumerate(combinations):
        sim_dir = create_simulation_directory(i, combo)
        run_simulation(sim_dir)
        extract_and_combine_data(sim_dir, 'combined_data.csv', i == 0)

    logging.info(f"Simulation generation completed. Results saved in {sim_folder}")

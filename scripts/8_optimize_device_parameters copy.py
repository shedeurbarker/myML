...
def optimize_parameters(original_params, models_data, method='both', maxiter=100):
    ...
    try:
        ...
        # Helper: clamp a vector into bounds
        def clamp_to_bounds(x):
            return [min(max(xi, b[0]), b[1]) for xi, b in zip(x, bounds)]

        # Helper: random feasible sample (uniform within bounds)
        def random_within_bounds():
            return [np.random.uniform(b[0], b[1]) for b in bounds]
        
        # Create initial guess from original parameters and clamp to bounds
        x0 = [original_params.get(name, (bounds[i][0]+bounds[i][1]) / 2.0) for i, name in enumerate(param_names)]
        x0 = clamp_to_bounds(x0)

        # Multi-start: generate additional random starts to escape flat regions
        multistarts = [x0]
        for _ in range(5):
            xr = random_within_bounds()
            multistarts.append(xr)
        ...
        if method in ['local', 'both']:
            ...
            try:
                for start_idx, start in enumerate(multistarts):
                    # Initialize progress tracking for each start
                    global _iteration_counter, _max_iterations, _optimization_method
                    _iteration_counter = 0
                    _max_iterations = maxiter
                    _optimization_method = f"Local (L-BFGS-B) [start {start_idx+1}/{len(multistarts)}]"
                    ...
                    if result_local.success and result_local.fun < best_objective:
                        ...
                        logging.info(f"Local optimization successful (start {start_idx+1}). Objective: {result_local.fun:.6f}")
            except Exception as e:
        ...
        if method in ['global', 'both']:
            ...
            try:
                ...
                _iteration_counter = 0
                _max_iterations = maxiter  # allow DE to use full iterations
